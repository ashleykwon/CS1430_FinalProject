...
def stereo_calibration(K_l, K_r, chessboard_images, chess_box_size_mm):
    ...
    for imgL, imgR in tqdm(chessboard_images):
        ...
        objp[:, :2] = np.mgrid[:10, :7].T.reshape(-1, 2)
        # objp = objp * chess_box_size_mm
        objpoints.append(objp)

        cornersL = cv2.findChessboardCorners(image=imgR, patternSize=(10, 7))[1]
        imgL_gray = cv2.cvtColor(imgL, cv2.COLOR_BGR2GRAY)
        cornersL = cv2.cornerSubPix(imgL_gray, cornersL, (11, 11), (-1, -1), criteria)
        imgpointsL.append(cornersL)

        cornersR = cv2.findChessboardCorners(image=imgL, patternSize=(10, 7))[1]
